...
import python_course_1604.class_01.exercize_06_while_break_else as esercize
...
class TestCaptureException(unittest.TestCase):
    ...
    def test_has_next(self):
        assert esercize.has_next() is True

    def test_get_next(self):
        assert "some_value" == esercize.get_next()

    def test_when_no_loops(self):
        has_next = self._mock_function(
            esercize, 'has_next', return_value=False)

        get_next = self._mock_function(esercize, 'get_next')

        with self.assertRaises(KeyError):
            esercize.count_until(7)
    ...
    def test_when_has_next_give_false_before_searched_value(self):
        has_next = self._mock_function(
            esercize, 'has_next', side_effect=iter(
                [True, True, False]))

        get_next = self._mock_function(
            esercize, 'get_next', side_effect=iter(range(10)))
